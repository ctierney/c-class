...
int main() {
	...
	int i = 0; 											/* to hold return values */
	int d = 0; 											/* to hold return values */
	...
	while ( (i = readline(line, MAX_LINE_LEN)) != 0) {
		...
		if (mode == PROCESS) {
			...
			/* change the delimiter if the line is a delim declaration */
			if ((d = check_delimiter(delim_ptr, line)) > 0) {

				if (DEBUG_ON)
					printf("debug: delim has been set to: %c\n", delim);

				continue;

			/* if encounter a noprocess tag, enter no process mode */
			} else if (strcmp(line, NO_PROCESS_OPEN) == 0) {
...
int check_delimiter(char *delim, char line[]) {
	...
	sscanf(line, "<delim value=%c />", &new_delim);

	if (new_delim != '\0') {

		if (DEBUG_ON) 
			printf("delim found! %c\n", new_delim);
